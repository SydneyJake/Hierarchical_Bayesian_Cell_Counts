...
# effective samples size and rhats
...
param_names <- c("theta\\[", "tau\\[", "gamma_raw\\[")
...
ggsave(filename = "diagnostics/neff_rhat.tiff", plot = p, dpi = 600, width = 0.6*5.2, height=2.6, units="in", compression="lzw")
...
ggsave(filename = "diagnostics/energy.tiff", plot = p, dpi = 600, width = 0.4*5.2, height=2.6, units="in", compression="lzw")
...
ggsave(filename = "diagnostics/bp_theta.tiff", plot = p, dpi = 600, width = 5.2-0.162, height=2.6/4, units="in", compression="lzw")

color_scheme_set(colorRampPalette(c("#beaed4", "black"))(8)[1:6])
p <- mcmc_trace(fit, bad_pars[2]) + legend_none() + ylab(expression(tau))
ggsave(filename = "diagnostics/bp_tau.tiff", plot = p, dpi = 600, width = 5.2-0.162, height=2.6/4, units="in", compression="lzw")

color_scheme_set(colorRampPalette(c("#fdc086", "black"))(8)[1:6])
p <- mcmc_trace(fit, bad_pars[3]) + legend_none() + ylab(expression(tilde(gamma)))
ggsave(filename = "diagnostics/bp_gamma.tiff", plot = p, dpi = 600, width = 5.2-0.162, height=2.6/4, units="in", compression="lzw")
